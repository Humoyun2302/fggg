    ...
    try {
      ...
      const mappedBarbers = barbers
        .map((b: any) => {
          ...
          // 1. subscription_status is 'active', OR
          // 2. subscription_expiry_date is in the future
          const now = new Date();
  ...
  const fetchProfile = async (userId: string, role: string) => {
    try {
      ...
      if (role === 'barber') {
        ...
        // CRITICAL: Use is_subscription_active directly from v_barber_subscription view
        // If view provides is_subscription_active, use it directly
        // Otherwise, calculate from ends_at as fallback
        let isSubscriptionActive = false;
        
        if (subscriptionData) {
          // Check if view has is_subscription_active field
          if (typeof subscriptionData.is_subscription_active === 'boolean') {
            isSubscriptionActive = subscriptionData.is_subscription_active;
            console.log('[FETCH PROFILE] ✅ Using is_subscription_active from view:', isSubscriptionActive);
          } else if (subscriptionData.ends_at) {
            // Fallback: calculate from ends_at
            isSubscriptionActive = new Date(subscriptionData.ends_at).getTime() > Date.now();
            console.log('[FETCH PROFILE] ⚠️ View missing is_subscription_active, calculated from ends_at:', isSubscriptionActive);
          }
        }
        ...
        const userFromDatabase = {
          ...
          priceRange: { min: minPrice, max: maxPrice },
          // CRITICAL: Use subscription data from subscriptions table
          subscriptionStatus: subscriptionData?.status || 'expired',
          subscriptionExpiryDate: subscriptionData?.ends_at || null,
