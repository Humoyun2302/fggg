...
if (isProduction) {
  ...
  const { projectId: devProjectId, publicAnonKey } = await import('./info.tsx');
  
  if (!devProjectId || !publicAnonKey) {
    const errorMsg = 'FATAL: Missing Supabase credentials. Please check Figma Make Supabase Integration.';
    console.error(errorMsg, { projectId: !!devProjectId, publicAnonKey: !!publicAnonKey });
    throw new Error(errorMsg);
  }
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    storageKey: 'soniya-auth-token', // Updated from 'trimly' to 'soniya'
  }
});
