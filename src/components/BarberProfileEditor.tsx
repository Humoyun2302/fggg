...
import { X, Upload, Save, Camera, MapPin, Languages as LanguagesIcon, DollarSign, ArrowLeft, Check, AlertTriangle, Baby } from 'lucide-react';
...
export function BarberProfileEditor({ barber, onClose, onSave }: BarberProfileEditorProps) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://${import.meta.env.VITE_SUPABASE_PROJECT_ID}.supabase.co/functions/v1/make-server-166b98fa/delete-barber/${barber.id}`, {
        method: 'DELETE',
        headers: {
      ...
      toast.success(t('barber.edit.accountDeleted'));
      ...
      toast.error(t('barber.edit.accountDeleteError'));
    } finally {
      setIsDeleting(false);
    }
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-6 space-y-4">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-[20px] border border-gray-200 dark:border-gray-700 mb-[16px] pt-[16px] pr-[16px] pb-[15px] pl-[16px] p-[16px]">
                  <button
                    onClick={() => setShowDeleteWarning(true)}
                    className="w-full h-11 bg-gradient-to-br from-red-500 via-red-600 to-rose-600 hover:from-red-600 hover:via-red-700 hover:to-rose-700 text-white rounded-[20px] shadow-[0_8px_30px_rgb(239,68,68,0.3),0_2px_10px_rgb(0,0,0,0.1)] hover:shadow-[0_12px_40px_rgb(239,68,68,0.4),0_4px_15px_rgb(0,0,0,0.15)] transition-all duration-300 flex items-center justify-center gap-2 font-medium backdrop-blur-sm"
                  >
      ...
      {showDeleteWarning && (
        ...
        >
          <motion.div 
            ...
            exit={{ scale: 0.9, opacity: 0 }}
            transition={{ type: 'spring', damping: 25, stiffness: 300 }}
            className="bg-white dark:bg-gray-800 p-6 rounded-[20px] shadow-2xl w-full max-w-md"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">
                  {t('barber.edit.deleteAccountWarningTitle')}
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  {t('barber.edit.deleteAccountWarningDesc')}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-3 justify-end mt-6">
              <Button
                ...
                className="bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 w-full sm:w-auto px-6"
              >
                {t('barber.edit.deleteAccountCancel')}
              </Button>
              <button
                onClick={handleDeleteAccount}
                disabled={isDeleting}
                className="w-full sm:w-auto px-6 h-11 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 font-medium"
              >
                {isDeleting ? (
                  <>
                  ...
                  <>
                    <AlertTriangle className="w-4 h-4" />
                    {t('barber.edit.deleteAccountConfirm')}
                  </>
                )}
              </button>
            </div>
