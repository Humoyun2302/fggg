...
export function SubscriptionManagement({
  ...
  // CRITICAL: Fetch subscription data directly from v_barber_subscription view
  const [dbSubscriptionData, setDbSubscriptionData] = useState<any>(null);
  const [isLoadingSubscription, setIsLoadingSubscription] = useState(true);
  
  // NEW: Fetch detailed subscription data from subscriptions table
  const [subscriptionDetails, setSubscriptionDetails] = useState<any>(null);
  
  useEffect(() => {
    const fetchSubscriptionFromView = async () => {
      if (!barberId) {
      ...
      try {
        console.log('[SUBSCRIPTION] 🔍 Fetching from v_barber_subscription for barberId:', barberId);
        ...
        if (error) {
          console.error('[SUBSCRIPTION] ❌ Error fetching from view:', error);
        } else {
          console.log('[SUBSCRIPTION] ✅ Data from v_barber_subscription:', data);
          setDbSubscriptionData(data);
        }
      } catch (err) {
    ...
    const fetchSubscriptionDetails = async () => {
      ...
      try {
        console.log('[SUBSCRIPTION] 🔍 Fetching detailed data from subscriptions table for barberId:', barberId);
        
        // Fetch the most recent subscription for this barber
        const { data, error } = await supabase
          .from('subscriptions')
          .select('*')
          .eq('barber_id', barberId)
        ...
        if (error) {
          console.error('[SUBSCRIPTION] ❌ Error fetching from subscriptions table:', error);
        } else {
          console.log('[SUBSCRIPTION] ✅ Detailed data from subscriptions table:', data);
          setSubscriptionDetails(data);
        }
      } catch (err) {
        console.error('[SUBSCRIPTION] ❌ Exception fetching subscription details:', err);
      }
    };
    
    fetchSubscriptionFromView();
    fetchSubscriptionDetails();
  }, [barberId]);
  
  // CRITICAL: Use database values as the single source of truth
  // Priority: dbSubscriptionData > props
  const finalCurrentPlan = dbSubscriptionData?.plan || currentPlan;
  const finalStatus = dbSubscriptionData?.status || subscriptionStatus;
  const finalExpiryDate = dbSubscriptionData?.ends_at || expiryDate;
  ...
  const handlePayWithMethod = async () => {
    ...
    try {
      ...
      const orderId = `SONIYA-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const amount = selectedPlan.totalPrice;
      const merchantName = 'Soniya Barber Booking';
